const handler = async (m, { Sky, text, prefix, command }) => {
    ...
    Sky.menfess = Sky.menfess ? Sky.menfess : {}
    ...
    let data = (await Sky.onWhatsApp(jid))[0] || {};
    if (!data.exists) throw 'Nomer tidak terdaftar di whatsapp.';
    if (jid == m.sender) throw 'tidak bisa mengirim pesan menfess ke diri sendiri.'
    
    let mf = Object.values(Sky.menfess).find(mf => mf.status === true);
    if (mf) return !0;
    ...
    await Sky.relayMessage(data.jid, {
        extendedTextMessage: {
            text: teks,
            contextInfo: {
